...
class TopRatedVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (self.movie.titles.count != 0){
            if let cell = self.tableView.dequeueReusableCell(withIdentifier: "TopRatedCell", for: indexPath) as? TopRatedCell
            {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.performSegue(withIdentifier: "TopRatedDetail", sender: self)
    }
    ...
    func requestTopRatedMovies(){
        self.movie = Movie()
        self.movie.top_rated = self
        movie.getAll(category: "top_rated")
    }
}
